...
| Set Warm Cutover Fact | `ansible.builtin.set_fact` | True |
...
classDef rescue stroke:#665352,stroke-width:2px;
  ...
  Set_Warm_Migration_Fact4-->|Task| Set_Warm_Cutover_Fact5[set warm cutover fact<br>When: **mtv migrate mtv warm   bool and mtv migrate warm<br>cutover time   default     true    length   0**]:::task
  Set_Warm_Cutover_Fact5-->|Task| Fail_When_Plan_Not_Ready_for_Migration6[fail when plan not ready for migration<br>When: **mtv migrate plan status migration   default    <br>true    length    0 and    mtv migrate plan status<br>conditions   default       selectattr  type   <br>defined     selectattr  status    defined    <br>selectattr  type    equalto    ready    <br>selectattr  status    equalto    true     list  <br>length    1**]:::task
  Fail_When_Plan_Not_Ready_for_Migration6-->|Task| Create_Migration7[create migration]:::task
  Create_Migration7-->|Include task| Monitor_Migration_Progress__monitor_migration_yml_8[monitor migration progress<br>When: **mtv migrate verify complete   bool**<br>include_task:  monitor migration yml]:::includeTasks
  Monitor_Migration_Progress__monitor_migration_yml_8-->End
```
