...
| [`mtv_plans_managed_by_label`](defaults/main.yml#L126)   | str   | `ansible-migration-factory` |  None  |   False  |  Managed By Label |
...
| [`mtv_plans_verify_delay`](defaults/main.yml#L135)   | int   | `20` |  None  |   False  |  MTV Migration Verify Plans Ready Delay |
| [`mtv_plans_verify_deleted`](defaults/main.yml#L116)   | bool   | `True` |  None  |   False  |  MTV Verify Deleted |
| [`mtv_plans_verify_ready`](defaults/main.yml#L121)   | bool   | `True` |  None  |   False  |  MTV Verify Ready |
| [`mtv_plans_verify_retries`](defaults/main.yml#L131)   | int   | `180` |  None  |   False  |  MTV Migration Verify Plans Ready Retries |

<summary><b>🖇️ Full descriptions for vars in defaults/main.yml</b></summary>
<br>
<b>`mtv_plans_action`:</b> Action to take. Choices include: create, archive or delete. Defaults to create.
<br>
...
<br>
<b>`mtv_plans_verify_ready`:</b> Verifies Plans are in a Ready state after creation
<br>
...
| create ¦ Set Labels for Verification | `ansible.builtin.set_fact` | True |
| create ¦ Verify Plans Ready | `ansible.builtin.include_tasks` | True |
...
#### File: tasks/verify_ready.yml

| Name | Module | Has Conditions |
| ---- | ------ | --------- |
| verify_ready ¦ Query Plans by Name | `kubernetes.core.k8s_info` | True |
| verify_ready ¦ Query Plans by Labels | `kubernetes.core.k8s_info` | True |
| verify_ready ¦ Combine and Check Results | `ansible.builtin.set_fact` | False |
| verify_ready ¦ Evaluate Ready Status | `ansible.builtin.set_fact` | False |
| verify_ready ¦ Fail if Retries Exceeded | `ansible.builtin.fail` | True |
| verify_ready ¦ Wait Before Retry | `ansible.builtin.pause` | True |
| verify_ready ¦ Retry Verification | `ansible.builtin.include_tasks` | True |
...
classDef rescue stroke:#665352,stroke-width:2px;
  ...
  create___Create_Plans0-->|Task| create___Set_Labels_for_Verification1[create   set labels for verification<br>When: **mtv plans verify ready   bool**]:::task
  create___Set_Labels_for_Verification1-->|Include task| create___Verify_Plans_Ready_verify_ready_yml_2[create   verify plans ready<br>When: **mtv plans verify ready   bool**<br>include_task: verify ready yml]:::includeTasks
  create___Verify_Plans_Ready_verify_ready_yml_2-.->|End of Block| create___Create_and_Verify_Plans22_block_start_0
  create___Verify_Plans_Ready_verify_ready_yml_2-->|Task| create___Display_Plans__Dry_Run_23[create   display plans  dry run <br>When: **mtv plans mtv dry run bool**]:::task
...
classDef rescue stroke:#665352,stroke-width:2px;
  ...
  Start-->|Task| verify_ready___Query_Plans_by_Name0[verify ready   query plans by name<br>When: **mtv plans plan name   default     true    length  <br>0**]:::task
  verify_ready___Query_Plans_by_Name0-->|Task| verify_ready___Query_Plans_by_Labels1[verify ready   query plans by labels<br>When: **mtv plans plan labels   default     true    length<br>  0**]:::task
  verify_ready___Query_Plans_by_Labels1-->|Task| verify_ready___Combine_and_Check_Results2[verify ready   combine and check results]:::task
  verify_ready___Combine_and_Check_Results2-->|Task| verify_ready___Evaluate_Ready_Status3[verify ready   evaluate ready status]:::task
  verify_ready___Evaluate_Ready_Status3-->|Task| verify_ready___Fail_if_Retries_Exceeded4[verify ready   fail if retries exceeded<br>When: **not   mtv plans all ready   bool and   mtv plans<br>verify ready retries count   int    mtv plans<br>verify retries   int**]:::task
  verify_ready___Fail_if_Retries_Exceeded4-->|Task| verify_ready___Wait_Before_Retry5[verify ready   wait before retry<br>When: **not   mtv plans all ready   bool and   mtv plans<br>verify ready retries count   int   mtv plans<br>verify retries   int**]:::task
  verify_ready___Wait_Before_Retry5-->|Include task| verify_ready___Retry_Verification_verify_ready_yml_6[verify ready   retry verification<br>When: **not   mtv plans all ready   bool and   mtv plans<br>verify ready retries count   int   mtv plans<br>verify retries   int**<br>include_task: verify ready yml]:::includeTasks
  verify_ready___Retry_Verification_verify_ready_yml_6-->End
```
